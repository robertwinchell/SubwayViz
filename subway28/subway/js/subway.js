...
/*				my.lineto.w -= dx;
				my.lineto.x += dx;
				my.lineto.anim(1, animtype, null, {ltr: true});
*/
...
/*			my.chl[0].lineto.w -= dx;
			...
			my.chl[0].lineto.anim(1, animtype, null, {ltr: true});
*/
...
/*		for(var ind in my.tree) {
			var shape = my.tree[ind];
			if(shape.lineto &&
				shape.lineto.x != shape.x) {
				shape.lineto.rmove(250,
					shape.x - shape.lineto.x, null,
					"linear", null, true);
			}
		}
*/
/*
		for(var ind in my.tree) {
			var shape = my.tree[ind];
			if(shape.lineto && shape.par) {
				if(shape.par.y == shape.y) { // horizontal line:
					if(shape.par.x > shape.x) {
						// right to left:
						if(shape.lineto.w !=
							(shape.par.x -
							shape.x
							...
							shape.lineto.anim(250,
							"linear", null,
							{ltr: true});
						}
					} else { // left-to-right:
						if(shape.lineto.x != shape.par.x
							|| shape.lineto.w !=
							(shape.x - shape.par.x))
									{
							shape.lineto.x =
				...
				} else { // vertical line or curve:
					if(shape.lineto.x != shape.x) {
						shape.lineto.rmove(250,
						shape.x - shape.lineto.x,
						null, "linear", null, true);
					}
				}
			}
		}
