...
$(document).mousemove(function(e) {
	if(M.over) {
		...
		M.tip.css({"left": e.clientX - 10 - M.tip.width(),
...
M.View = function(div) {
	...
	my.draw = function() {
			...
			for(j = 0; j < row.circle.length; ++j) {
				...
				if(!c.ispoint) {
					...
					var dtext = $('<p>')
						.text(c.description);
					descr.append(dtext);
					my.canvas.append(descr);
					descr.css({height: my.descrh + 'em',
						width: my.descrw + 'em'});
					// actual descr height:
					var realh = $(descr.height()).toEm({
						scope: descr});
					descr.css({left: (x - (my.descrw *
							0.5) + my.radius)
									+ 'em',
							top: (y - realh -
								my.descryoff)
		...
		if(my.ismap) {
			my.container.css({"right": (my.xmax + 1) + 'em',
					"top": 1 + 'em'});
		}
	}
}
...
M.navbox = $('#minimap .navbox'); // access to box inside the minimap
M.mapv = $('#minimap'); // minimap container
M.mapc = $('#minimap .canvas'); // access to the minimap canvas
...
$('#mainview').scroll(M.updatenavbox);
...
$(window).mousedown(function(e) {
	...
	M.startx = e.screenX;
	M.starty = e.screenY;
	M.vx = $('#mainview').scrollLeft();
	M.vy = $('#mainview').scrollTop();
	M.mousedown = true;
...
$(window).mousemove(function(e) {
	if(M.mousedown) {
		...
		$('#mainview').scrollLeft(M.vx + M.dx);
		$('#mainview').scrollTop(M.vy + M.dy);
	//	M.updatenavbox();
	}
});
...
M.navigate = function(e) {
		var mmx = e.pageX - M.mapv.position().left; // minimap x
		var mmy = e.pageY - M.mapv.position().top; // minimap y
