...
M.Node = function(model, id) {
	...
	my.get_colors = function() {
		...
		if(my.abc) { // this is a subnode,
			...
			var col = my.get_node(my.grp + my.num).colors;
			if(col) {
				return col;
			}
		}
		// check if the group has colors:
		var col = my.get_node(my.grp).colors;
		if(col) {
			return col;
		}
	...
	my.mkrows = function(row, depth) {
		if(!my.par_id) { // we are at root node: create zeroth row
			my.model.row = [];
			depth = 0;
...
M.Line = function(o) {
	...
	my.anim = function(time, animtype, callafter) {
		if(my.par.x > my.x) console.log(my.par.x);
