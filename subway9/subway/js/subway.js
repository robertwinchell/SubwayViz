...
M.Shape = function(o) {
	...
	my.descrh = 2;
	my.descroff = 0.3; // description vertical offset from the shape
	...
	my.linew = (o.linew !== undefined)? o.linew: 0.3; // line width, em
	my.div; // outer div
	...
	my.select = function() {
		my.inner_div.css({background: "#ff0"});
	};
	my.deselect = function() {
		my.inner_div.css({background: my.fill_color});
	};
	...
	my.draw = function(parent_div) {
		my.div = $('<div>').addClass("octagon").
			css({
				...
				"background-image":
				"linear-gradient(45deg, " + my.stroke_color +
				',' + my.stroke_color +
				" 37.5%, transparent 37.5%, transparent 62.5%, "
				+ my.stroke_color + " 62.5%, " +
				my.stroke_color +
		...
		my.inner_div = $('<div>').addClass("octagon_inner")
			.css({ "background-image":
				"linear-gradient(45deg, " + my.fill_color + ','
				+ my.fill_color +
				" 37.5%, transparent 37.5%, transparent 62.5%, "
				+ my.fill_color + " 62.5%, " +
				my.fill_color + " 100%),linear-gradient(-45deg,"
...
M.Line = function(o) {
	...
	my.deselect = function() {
		my.inner_div.css({background: my.stroke_color});
	};
	return my;
};
	...
	my.anim = function(shape) { // shape from my.root
		...
		var shape; // this can be a circle, octagon or a point
			...
			if(c.type == 'badge') {
				...
				.draw(my.canvas).set_descr(c.description);
			} else {
				shape = new M.Circle(options)
				.draw(my.canvas).set_descr(c.description);
			}
                        if (my.ismap) {
                            M.addtip(shape.div, c.description);
                        } else {
...
//			descr.hide();
			...
			shape.chl = []; // array of links to next circ divs
			...
			// update the bounding values:
                    my.xmax = ((x + shape.w * 0.5 + my.xoff) > my.xmax ?
			...
			shape.lineto = line; // line to this circle in DOM tree
                    }
                }
            }
