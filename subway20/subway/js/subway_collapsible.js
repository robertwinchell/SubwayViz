...
*/
	...
	my.rmove = function(time, dx, dy, animtype, callafter, nonrec) {
		my.x += dx;
		my.y += dy;
		my.div.animate({
		...
		for(var i = 0; i < my.chl.length; ++i) {
			my.chl[i].rmove(time, dx, dy, animtype);
		}
	}
...
*/
	...
	my.expand = function(id) {
		my.tree[id].chl[0].rmove(250, my.xstep, 0, "swing", function() {
			my.tree[id].chl[0].anim(100);
			my.fit();
	...
	my.collapse = function(id) {
		...
		my.tree[id].rmove(250, -my.xstep, 0, "swing",
								function(){
...
M.convert_raw = function(input) {
	...
	for(var i = 0; i < input.DiagramNodes.length; ++i) {
		...
		// if this is a branch, we are sure starting a new supergroup:
		var o = {oid: in_node.DiagramNodeId}; // options
		if(in_node.ParentDiagramNodeId &&
				in_node.ParentDiagramNodeId != prev_oid) {
...
//			o.colors = [fill_color, stroke_color , line_color];
		if(cur_num == 1) {
